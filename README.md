...
- [Phase 6 发布验收表](docs/phase6-release-checklist.md)
- [8分钟比赛演示脚本](docs/demo-script.md)
- [比赛展示材料提纲](docs/competition-showcase.md)
- [已知限制与上线门禁](docs/known-limitations.md)
...
- [统一聊天、AI工单交接与流式输出契约](docs/unified-chat-routing-contract.md)
...
普通聊天最多50轮、单条16000字、答案64000字，按问题复杂度回答而非强制短答。模型只收到最近最多12对
问答且正文≤120000字，省略旧历史时页面提示；会话累计内容到400000字后下一次发送要求新开问题。
具体预算与兼容规则见[深度聊天契约](docs/deep-chat-contract.md)和[统一聊天契约](docs/unified-chat-routing-contract.md)。
旧`consult` API仍最多6轮/单条1000字；
服务端最多200个内存会话，30分钟不活跃后惰性过期。未保存咨询在页面刷新/离开
...
聊天输入框支持单张图片附件（JPEG/PNG，5MiB 上限）：选择或拖入后在输入框内显示可移除的缩略图，
发送时先上传到原有 `/api/photos` 取得图片 ID，再与文字合并成同一次聊天请求。

- 可以只发图片：服务端补一句默认描述，工单流程与状态机不变；`/consult` 咨询入口不接受图片。
- 授权逐次有效：每次发送都要勾选“图片已打码且有权上传”；Mock 模式不识图，也不外发。
- 真实外发使用 DeepSeek 官方多模态消息（`deepseek-flash`，`image_url` 数据 URL），仍走原有工具循环。
- 会话历史只保存图片 ID，不保存 base64；历史轮次的图片不会重发，只保留文字说明。
- 含图片的工程类分析强制人工复核；单张图片不能作为安全、质量或责任的最终认定。

独立真实验收脚本 `scripts/verify-chat-multimodal-live.py` 默认拒绝联网，必须显式传 `--allow-external`
才调用付费模型；它最多三轮、只生成无隐私的合成图形，并且不读取或修改 `.env`。普通 `verify.ps1` 不执行它。
