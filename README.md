...
本地整改工作流和只读知识检索，并完成 Phase 5 单图证据能力及文字/历史工单产品补齐。
文字与图片均有独立模型入口和逐次外发确认；真实文字调用尚未独立冒烟，现场效果仍需专业验收。
当前是本机受控演示产品，不是具备生产身份鉴权的上线系统。

## 当前能力

- Vue 3 + TypeScript + Vite 咨询首页、历史工单列表和可刷新详情。
...
启动前后端后，首页“描述你遇到的问题”接收自然语言，不需手动选择风险或类别。
real模式每次发送前都要确认将当前消息、标签及本会话历次用户补充发给DeepSeek并承担费用。
文字沿用 `LLM_MODEL=deepseek-v4-flash`（也支持已核验的v4-pro）及现有密钥/官方HTTPS端点；
30秒超时、零自动重试、每轮一次请求、最多4096输出token。无需改变VISION_MODEL。

普通咨询只回答；信息不足显示追问；四类明确需跟进问题才提供“基于最新分析生成待确认提案”。
...
Mock模式明确不执行真实文字理解；如只需离线演示流程，可展开“结构化流程演示（不调用模型）”。
