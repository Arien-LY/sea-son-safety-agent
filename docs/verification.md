...
## 2026-09-21：beta.4 打包

目标：按最新主线0f17e5e构建新版Windows软件包；用户已公开仓库，保留旧发行包。
教程依据：第12章离线验证、第16章可复现交付。只修改发布说明、客户指南与验证脚本；不改产品业务。
已核对9月18日图片聊天与9月20日TicketDecision/Word导出/本地删除提交；Git作者均为Arien-LY。
补齐开发环境锁定的python-docx/lxml；修复聊天菜单测试对Windows CRLF的误判（仅测试文本规范化）。
verify.ps1：586项Python通过，1项因缺少本地公开案例源目录而跳过；65项前端通过，生产构建通过。
发布门禁：干净提交构建、独立解压原生启动/包内运行时、散列及Word导出验证；不调用付费模型。

2026-09-16 已知问题修复：539 Python / 44 前端、构建及隔离真实 API/浏览器复测通过；详见 [复测报告](acceptance/2026-09-16-fixes/README.md)，保留来源语义与分析降级限制。

## 2026-09-16：真实 API 验收结果归档

- 完整报告及必要脱敏证据：[真实 API 验收](acceptance/2026-09-16-real-api/README.md)。
- 验收源提交 d31d4fb，PR #28 已合入 main（09cb5f6），两者文件树一致；本次仅归档文档，不宣称修复产品。
- 35次DeepSeek文字请求、1次合成图片调用；4项主要缺陷未关闭，Tavily缺凭据未测。
- 真流式、工具结果回填、重启幂等、图片独立授权、安全合成工单闭环有成功证据；详细失败与局限见报告。
- 验收阶段全量验证：523项Python、43项前端及生产构建通过；离线结果不能替代真实失败闭环。
- 用户在验收完成后明确授权上传并合并；归档排除凭据、运行数据库、进程文件和收费调用启动器。

## PR #28 主线冲突修复

- GitHub报告CONFLICTING/DIRTY。main的6507387是旧打包改动的压缩提交，其文件树与功能分支祖先2d9640c完全一致（git diff为空）。
- 在功能分支合入origin/main，12处冲突保留已验证的后续版本；解决后与原HEAD代码树一致，没有丢失main独有内容。
- 范围仅合并历史、交付记录和PR说明；不升级依赖、不改业务权限、不强推、不自动合并PR。
- 验证通过：无未解决冲突、git diff --check；全量scripts/verify.ps1为523项Python、43项前端和生产构建通过。
...
## 聊天 composer 本地附件预览（2026-09-18）

- 修正目标：选图或拖放后只在输入框内部显示可移除的缩略图，不在聊天中展示图片证据、上传、授权或分析面板。
- 教程依据：第 7/9 章的职责边界和最小实现；本轮不扩展模型或工具能力。
- TextPanel 独立管理 File、object URL 和附件校验错误；原生选择器和聊天窗口/输入框拖放共用同一个选择方法。保持单张 JPEG/PNG、5MiB 上限，非法输入明确拒绝并保留原图。
- HomeView 不再为聊天挂载 ImagePanel；ImagePanel 恢复原有实现，继续用于工单证据等原有独立场景。没有新增上传链路。
- 预览完全本地：不会上传、请求图片 runtime 或分析；文字请求协议不变，仅发送文字，发送后保留尚未发送的附件。移除、替换、新建/切换会话和组件卸载释放预览 URL。
- busy/恢复状态禁止选图、拖入与移除。模型、Prompt、Agent、后端图片协议、工单状态机及 .env 未修改。
- 完整 scripts/verify.ps1 通过：539 项 Python 测试、48 项前端测试、vue-tsc 与 Vite production build；保留 1 条既有 Pydantic 弃用警告。git diff --check 通过。操作系统文件选择器、真实鼠标拖放与布局仍需人工浏览器验收。
- 后续多模态尚需图文请求契约、附件会话持久化/恢复、模型图片消息接入、外发授权及幂等重试安全验证。本轮不实现。


## 聊天图片附件接入多模态请求链（2026-09-18）

- 修正目标：让 composer 已有的缩略图附件真正进入“前端 → FastAPI → DeepSeek 多模态 → 原 Agent/tool_loop”，
  支持纯文字、图文和只发图片三种用法；单张 JPEG/PNG ≤5MiB；消息中显示本轮图片；兼容旧会话。
  上一节遗留的图文契约、持久化/恢复、模型图片消息、外发授权与幂等重试在本节完成。
- 教程依据：第 7 章原生工具调用、第 9 章上下文预算、第 12 章 Fake 评估，以及 Extra09 最小闭环与留痕。
- 请求链路：TextPanel 选择/拖放 → 发送时先 `POST /api/photos`（原上传边界：真实格式检查、Pillow 安全解码、
  清除元数据、标准化 JPEG，4096 边长/1600 万像素上限）取得 `photo_id` → 同一次 `POST /api/text-consultations(/stream)`
  携带 `photo_id` + `allow_image_external` → 聊天服务用与图片路由共享的 PhotoStore 复核 ID 与完整性 →
  原 `TextAssistant` 与工具循环 → 响应回带 `photo_id`，前端在用户消息内显示该图片。
- 官方多模态格式（2026-09-18 核对 `https://api-docs.deepseek.com/guides/vision/`）：DeepSeek Chat Completions，
  `model=deepseek-flash`，最后一条 user 消息内容为数组
  `[{"type":"text","text":…},{"type":"image_url","image_url":{"url":"data:image/jpeg;base64,…","detail":"original"}}]`；
  图片只来自本地 PhotoStore 已重编码的 JPEG，不接受公网图片 URL，不绕过原视觉配置门禁。
- 契约与兼容：新增产品文字请求/响应 v6 快照（v1–v5 不变）；`photo_id` 与 `allow_image_external` 为可选字段，
  `intent=consult` 拒绝图片；只发图片时服务端补默认描述；旧纯文字幂等哈希排除新增默认字段，
  会话只持久化图片 ID，不落盘 base64；历史轮次不重发图片，仅在上下文附文字说明。
- 边界：real 模式必须本次勾选图片外发授权且 `VISION_MODEL` 已配置，否则 403/503 拒绝；
  Mock 不调用图片模型、界面也不声称使用视觉模型（发送仍使用本地模型标识）；
  含图片的工程类分析强制 `requires_human_review=true`、`recommended_route=human_review` 并补充不确定性说明；
  工单状态机、确认权限与 `.env` 未改。
- 自动验证：`scripts/verify.ps1` 全量通过——550 项 Python 测试、53 项前端行为测试、
  `vue-tsc` 与 Vite 生产构建（68 modules）成功，保留 1 条既有 Pydantic 弃用警告。
  专项 `tests/test_multimodal_chat.py` 11 项离线 Fake 测试，覆盖图片经原生工具循环后仍原样回传、
  历史恢复只含文字说明且不落盘 base64、只图默认描述与严格字段、缺授权/缺图片/缺视觉配置/图片损坏、
  上传 API 复用与 ID 传递、旧幂等哈希不变、图片工程分析强制人工复核；前端新增图文、只图、失败重试、
  取消上传、切换会话丢弃过期上传、刷新恢复、Mock 标识与客户文案等行为测试。
- 本轮同时修复两处真实失败：上一轮遗留的模型选择源码断言（现按“文字/Mock 用所选模型，仅 real 图片轮用图片模型”
  冻结），以及客户界面模板禁用词 `Mock`（上下文中已存在的授权文案与新增提示均改为既有客户措辞
  “AI 服务未启用”）。未删除或弱化任何业务断言来换通过。
- 真实付费验收（用户已授权，显式 `--allow-external` 才执行，最多三轮，仅合成无隐私图形，不读不改 `.env`）：
  1) 图文 + 工具：正确辨认左红色正方形、右蓝色圆并读出图内算式 17*23，真实调用 `calculate` 得到 391，3.56 秒；
  2) 只发图片：正确辨认白底绿色三角形，5.8 秒；
  3) 同会话后续纯文字：`PURE_TEXT_OK`，1.11 秒。
  原始结果在本地运行数据 `data/chat-multimodal-live.json`（不提交、无真实个人信息、未创建工单）。
- 未完成的验证：真实浏览器鼠标拖放与操作系统文件选择器仍未人工验收；`web_search`/`search_knowledge`
  在图片轮次仅有 Fake SDK 覆盖（当前未配置 Tavily，真实搜索本轮未成功，不作通过声明）；
  历史轮次图片不重发，只保留文字说明；单张图片不构成安全、质量或责任认定，真实工地准确率仍需专业抽样验收。


## 最近聊天置顶与删除（2026-09-18）

- 目标/允许范围：AppShell 菜单、会话API/SQLite元数据、相关测试和文档；不改模型、Agent、图片流程、工单状态机或 .env，不 push。
- 教程依据：第7/9章，界面触发确定性应用服务操作，模型不拥有删除或置顶权限。
- 专项：tests/test_chat_menu.py 验证旧快照缺省、重启后置顶排序、取消置顶恢复原顺序、后续保存保留置顶、SQL真实删除、过期续聊不复活、存储失败不改内存、运行中写互斥、API严格类型及删除确认。
- frontend/tests/chat-menu.test.cjs 验证菜单不导航、置顶/取消、取消删除无副作用、当前/非当前路由、导航保护拒绝删除、失败提示、过期列表不恢复已删项、busy不重复触发。
- 完整 scripts/verify.ps1：556 项 Python 测试、59 项前端测试及 vue-tsc/Vite production build 全部通过；git diff --check 通过。保留 1 条既有 Pydantic 弃用警告。真实浏览器点击与原生确认框尚未人工验收。所有自动测试使用Fake/Mock，不调用真实模型。
